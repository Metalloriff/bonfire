extends AudioStreamPlayer

func _ready() -> void:
	var str: AudioStreamOpusChunked = stream

	var fin = FileAccess.open("res://test.ogg", FileAccess.READ)
	var oggsblocks = []
	for i in range(99):
		var gg = {}
		gg["oggs"] = fin.get_buffer(4)
		if gg["oggs"] != "OggS".to_ascii_buffer():
			print("not ogg")
			break
		gg["zero"] = fin.get_8()
		gg["bitflags"] = fin.get_8()
		gg["absolute_granule_position"] = fin.get_64()
		gg["stream_serial_number"] = fin.get_32()
		gg["page_sequence_number"] = fin.get_32()
		gg["page_checksum"] = fin.get_32()
		gg["page_segments"] = fin.get_8()
		gg["segment_table"] = fin.get_buffer(gg["page_segments"])
		var packet_size = 0
		for s in gg["segment_table"]:
			packet_size += s
		gg["buff"] = fin.get_buffer(packet_size)
		oggsblocks.push_back(gg)

	print(len(oggsblocks), " blocks")
	var oh = oggsblocks[0]["buff"]
	assert(oh.slice(0, 8) == "OpusHead".to_ascii_buffer())
	var ohd = {"version": oh[8],
				"channelcount": oh[9],
				"preskip": oh.decode_u16(10),
				"sample_rate": oh.decode_u32(12),
				"outputgain": oh.decode_u16(16)}
	print(ohd)
	stream.opus_channels = ohd["channelcount"]
	stream.opus_sample_rate = ohd["sample_rate"]
	play()
	for gg in oggsblocks.slice(3, 8):
		print([gg["absolute_granule_position"], gg["page_sequence_number"]])
	
	var audiostreamplaybackopus: AudioStreamPlaybackOpusChunked = get_stream_playback()
	
	for gg in oggsblocks.slice(3):
		print(gg["segment_table"])
		var bbs = buffblocks(gg["segment_table"], gg["buff"])
		print("bbs ", len(bbs))
		for bb in bbs:
			while 48000 * 60 / 1000 > audiostreamplaybackopus.available_space_frames():
				await get_tree().create_timer(0.1).timeout
			audiostreamplaybackopus.push_opus_packet(bb, 0, 0)

	# var thread := Thread.new()
	# thread.start(_pipe)
	
func buffblocks(st, buff):
	var siprev = 0
	var si = 0
	var buffblocks = []
	for s in st:
		si += s
		if s != 255:
			buffblocks.push_back(buff.slice(siprev, si))
			siprev = si
	return buffblocks

func _pipe() -> void:
	var str: AudioStreamOpusChunked = stream

	# do command: yt-dlp -f bestaudio -o - -q --no-progress --limit-rate 32K "https://youtu.be/dT2NWgP-vO4"
	var output: PackedByteArray = []
	OS.execute("yt-dlp", ["-f", "bestaudio", "-o", "-", "-q", "--no-progress", "https://youtu.be/dT2NWgP-vO4"], output)

	print("done")
	
	str.push_opus_packet(output, 0, 0)